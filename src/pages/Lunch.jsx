...
const Lunch = () => {
  ...
  const [bgImage, setBgImage] = useState("bg-image-33");
  const [loaded, setLoaded] = useState(false);
  const navigate = useNavigate();
  const { products } = useContext(ShopContext);

  const navLinks = [
    { text: "Breakfast", path: "/breakfast", bg: "bg-image-2" },
    { text: "Lunch & Dinner", path: "/lunch", bg: "bg-image-3" },
    { text: "Snacks & Snaps", path: "/snacks", bg: "bg-image-4" },
    { text: "Desserts", path: "/desserts", bg: "bg-image-5" },
    { text: "Drinks", path: "/drinks", bg: "bg-image-6" },
    { text: "Menu", path: "/menu", bg: "bg-image-7" },
  ];
  ...
  const lunchItems = Array.isArray(products?.lunch) ? products.lunch : [];

  // Extracted Product Card Renderer with animations
  const renderProductCard = (product, index) => (
    <div
      key={index}
      className={`border-[#423f32] border-2 w-[280px] rounded-lg p-5 bg-[#423f32] shadow-lg hover:shadow-xl transition-transform transform hover:scale-100 duration-500 opacity-0 translate-y-4 ${
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-8 pt-16 w-full max-w-6xl">
          ...
          <div className="flex flex-col items-center md:items-start flex-1">
            <div className="w-[900px] h-[470px] overflow-y-auto no-scrollbar">
              {lunchItems.length === 0 ? (
                <p className="text-gray-700">No lunch items available.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {lunchItems.map(renderProductCard)}
                </div>
              )}
            </div>
          </div>
