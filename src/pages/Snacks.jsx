...
const Snacks = () => {
  ...
  const [bgImage, setBgImage] = useState("bg-image-44");
  ...
  const snacksItems = Array.isArray(products?.snacks) ? products.snacks : [];

  const renderProductCard = (product, index) => (
    <div
      key={index}
      className={`border-[#423f32] border-2 w-[280px] rounded-lg p-5 bg-[#423f32] shadow-lg hover:shadow-xl transition-transform transform 0 duration-500 opacity-0 translate-y-4 ${
            ...
            <div className="w-[900px] h-[470px] overflow-y-auto no-scrollbar">
              <div className="mb-12 scroll-mt-20">
                {snacksItems.length === 0 ? (
                  <p className="text-gray-700">No snacks items available.</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {snacksItems.map(renderProductCard)}
                  </div>
                )}
              </div>
            </div>
