...
const Desserts = () => {
  ...
  const [bgImage, setBgImage] = useState("bg-image-55");
  ...
  const dessertsItems = Array.isArray(products?.desserts)
    ? products.desserts
    : [];
            ...
            <div className="w-[900px] h-[470px] overflow-y-auto no-scrollbar">
              <div className="mb-12 scroll-mt-20">
                {dessertsItems.length === 0 ? (
                  <p className="text-gray-700">No dessert items available.</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {dessertsItems.map(renderProductCard)}
                  </div>
                )}
              </div>
            </div>
